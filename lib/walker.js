...
Walker.prototype = {
    ...
    fire: function(typeSpec, node, childNode) {
        ...
        if (evtList.length) {
            var parts = /^(?:(before|after)?:)?(\w+)(?:\.(\w+))?$/.exec(
                typeSpec
            );
            ...
            result = evtList.every(function(a) {
                return false ===
                    a.fn.apply(
                        a.scope || this,
                        [node, childNode].concat(
                            parts ? parts.slice(1) : [typeSpec]
                        )
                    )
                    ? false
                    : true;
    ...
    on: function(typeSpec, cb, scope) {
        ...
        if (_.isArray(typeSpec)) {
            ...
            var self = this;
            _.forEach(typeSpec, function(f, t) {
                self.on(t, f, scope);
            });
        } else {
            var parts = /^(?:(before|after):)?(\w+|\*)((?:\.)\w+)?$/.exec(
            ...
            if (!parts) {
                throw Error(
                    "Invalid format for parse node specified: " + typeSpec
                );
            }
