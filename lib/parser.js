...
var token_error = function(t, message) {
  ...
  throw new ParseError(
    t,
    "Error at '" + t.value + "' on line " + line + ": " + message
  );
};
...
function makeParser(options) {
  ...
  var original_scope = {
    ...
    find: function(n) {
      ...
      while (true) {
        ...
        if (!e) {
          o = symbol_table[n];
          var rtn = o && typeof o !== "function" ? o : symbol_table["(name)"];
          return rtn;
        }
      }
    },
  ...
  function reverse() {
    token_nr -= 2;
    console.log("Reversing: ", token_nr);
    advance();
  }
      ...
      case "name":
        if (_.indexOf(builtinTypes, vl) !== -1) {
          ...
          if (o === symbol_table["(name)"]) {
            ...
            // This somehow found the root symbol
            // when it should have found the built-in type.

            console.log("override of builtin type? ", v);
            o = symbol_table[vl];
            a = "name";
          }
        } else {
          ...
          setDecl = true;
          decl = o !== symbol_table["(name)"];

          if (setDecl) {
            console.log(v, "setting decl to ", decl);
            console.log("scope ", scope, " o is ", o);
          }
        }
  ...
  return {
    ...
    parse: function parse(source) {
      ...
      if (
        tokens.length &&
        (token.value !== "function" || token.arity !== "name")
      ) {
