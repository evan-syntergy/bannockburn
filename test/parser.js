...
describe('Parser', function(){
  describe('#parse()', function(){
    ...
    it( 'should parse empty functions', function() { 
      comp(  Parser.parse( "function x(); end" ), [{ type: "FunctionDeclaration", name: "x", body: null }] );
    });
    it( 'should parse functions with parameters', function() { 
      comp( Parser.parse( "function x(p1,p2); end" ), [{ type: "FunctionDeclaration", name: "x", body: null, params: [ { name: { value: "p1" } }, { name: { value: "p2" } } ] }] );
    });
    it( 'should parse functions with parameters that have default values', function() { 
      comp( Parser.parse( "function x(String p1,Integer p2=1); end" ), [{ type: "FunctionDeclaration", name: "x", body: null, params: [ { dataType: "String", name: { value: "p1" } }, { name: { value: "p2" }, default: { value: "1" } } ] }] );
    });
    it( 'should parse functions with variable arguments', function() { 
      comp( Parser.parse( "function x(String p1,Integer p2=1, ... ); end" ), [{ type: "FunctionDeclaration", name: "x", body: null, params: [ { dataType: "String", name: { value: "p1" } }, { name: { value: "p2" }, default: { value: "1" } },  { id: "..." } ] }] );
    });
    ...
    it( 'should parse switch statement with case with multiple tests', function() { 
      comp( Parser.parse( "switch i; case 1, 2\n echo( 'test' ); end; end;" ),
        [ { type: "SwitchStatement", cases: [ { test: [ { value: 1 }, { value: 2 } ] } ] } ] );
    });
    ...
    it( 'should parse if statements', function() { 
      comp( Parser.parse( "if i == 23; end;" ),
        [ { type: "IfStatement", test: { type: 'RelationalExpression' }, consequent: null } ] );
    });
    it( 'should parse if/else statements', function() { 
      comp( Parser.parse( "if i == 23; echo( true ); else; echo( false ); end;" ),
        [ { type: "IfStatement", 
    ...
    it( 'should parse if/elseif statements', function() { 
      comp( Parser.parse( "if i == '23' + \"fdsa\"; echo( true ); elseif y; echo( false ); end;" ),
        [ { type: "IfStatement", 
            test: { type: 'RelationalExpression' }, 
            consequent: [{ type: 'ExpressionStatement' }], 
    ...
    it( 'should parse if/elseif/else statements', function() { 
      comp( Parser.parse( "if i == 23.0 + 123.001 + .1 + 0.1; echo( true );  elseif y; echo( false );else;echo('else'); end;" ),
        [ { type: "IfStatement", 
            test: { type: 'RelationalExpression' }, 
            consequent: [{ type: 'ExpressionStatement' }], 
    ...
    it( 'should parse function calls', function() { 
      comp( Parser.parse( "x()" ),
        [ { type: "ExpressionStatement", expression: { type: "CallExpression", callee: { value: 'x' } } } ] );
    });
    it( 'should parse function calls with one argument', function() { 
      comp( Parser.parse( "x(1)" ),
        [ { type: "ExpressionStatement", expression: { type: "CallExpression", callee: { value: 'x' }, arguments: [ { value: "1" } ] } } ] );
    });
    it( 'should parse function calls with multiple arguments', function() { 
      comp( Parser.parse( "x(1,2)" ),
        [ { type: "ExpressionStatement", expression: { type: "CallExpression", callee: { value: 'x' }, arguments: [ { value: "1" }, { value: "2" } ] } } ] );
    });
    it( 'should parse member expressions', function() { 
      comp( Parser.parse( "x.y" ),
        [ { type: "ExpressionStatement", expression: { type: "MemberExpression", object: { value: 'x' }, property: { value: 'y' } } } ] );
    });
    it( 'should parse complex member expressions', function() { 
      comp( Parser.parse( "x.( a + b + c + y )" ),
        [ { type: "ExpressionStatement", expression: { type: "MemberExpression", computed: true, object: { value: 'x' }, property: { type: 'BinaryExpression' } } } ] );
    });
    it( 'should parse member expression calls', function() { 
      comp( Parser.parse( "x.( a + b + c + y )()" ),
        [ { type: "ExpressionStatement", expression: { type: "CallExpression", callee: { type: 'MemberExpression', computed: true } } } ] );
    });
    it( 'should parse member expression calls with arguments', function() { 
      comp( Parser.parse( "x.( a + b + c + y )(1,2,3)" ),
        [ { type: "ExpressionStatement", expression: { type: "CallExpression", callee: { type: 'MemberExpression', computed: true }, arguments: [ {value:"1"}, {value:"2"}, {value:"3"} ] } } ] );
    });
    ...
    [ "+", "-", "*", "/", "%", "&", "|", "^", "in" ].forEach( function( op ) { 
        it( 'should parse binary operator ' + op, function() { 
          comp( Parser.parse( "x = y " + op + " z" ),
            [ { type: "ExpressionStatement", expression: { type: "AssignmentExpression", right: { type: "BinaryExpression", operator: op } } } ] );
        });
    } );
    [ "<", ">", "<=", ">=", "==" ].forEach( function( op ) { 
        it( 'should parse relational operator ' + op, function() { 
          comp( Parser.parse( "x = y " + op + " z" ),
            [ { type: "ExpressionStatement", expression: { type: "AssignmentExpression", right: { type: "RelationalExpression", operator: op } } } ] );
        });
    } );
    [ "&&", "||", "^^" ].forEach( function( op ) { 
        it( 'should parse logical operator ' + op, function() { 
          comp( Parser.parse( "x = x " + op + " z" ),
            [ { type: "ExpressionStatement", expression: { type: "AssignmentExpression", right: { type: "LogicalExpression", operator: op } } } ] );
        });
    } );
    [ [ "lt", "<" ], [ "gt", ">"], ["and", "&&"], [ "or", "||" ], ["xor","^^"] ].forEach( function( op ) { 
    ...
    [ [ "not", "!" ] ].forEach( function( op ) { 
        it( 'should parse alternate unary operator ' + op[0], function() { 
          comp( Parser.parse( "x =      " + op[0] + " z" ),
            [ { type: "ExpressionStatement", expression: { type: "AssignmentExpression", right: { type: "UnaryExpression", operator: op[1] } } } ] );
        });
    } );
    it( 'should parse ternary expressions', function() { 
      comp( Parser.parse( "return x == 23 ? 1 : 2" ),
        [ { type: "ReturnStatement", argument: { type: "ConditionalExpression", test: { type: 'RelationalExpression' }, consequent: {}, alternate: {} } } ] );
    });
    it( 'should parse global references', function() { 
      comp( Parser.parse( "$g.test" ),
        [ { type: "ExpressionStatement", expression: { type: "MemberExpression", object: { type: "UnaryExpression", id: "$" } } } ] );
    });
    ...
    it( 'should parse break statement', function() { 
      comp( Parser.parse( "break" ),
        [ { type: "BreakStatement" } ] );
    });
    it( 'should parse continue statement', function() { 
      comp( Parser.parse( "continue" ),
    ...
    it( 'should parse continueif statement', function() { 
      comp( Parser.parse( "continueif 123 > 2" ),
        [ { type: "ContinueIfStatement", argument: { type: 'RelationalExpression' } } ] );
    });
    it( 'should parse index specifier', function() { 
      comp( Parser.parse( "array[123]" ),
        [ { type: "ExpressionStatement", expression: { type: "IndexExpression", index: {value: 123} } } ] );
    });
    it( 'should parse range specifier', function() { 
      comp( Parser.parse( "string[1:15]" ),
        [ { type: "ExpressionStatement", expression: { type: "RangeExpression", fromIndex: {value: 1}, toIndex: { value: 15 } } } ] );
    });
    it( 'should parse range specifier with empty start', function() { 
      comp( Parser.parse( "string[:15]" ),
        [ { type: "ExpressionStatement", expression: { type: "RangeExpression", fromIndex: null, toIndex: { value: 15 } } } ] );
    });
    it( 'should parse range specifier with empty end', function() { 
      comp( Parser.parse( "string[1:]" ),
        [ { type: "ExpressionStatement", expression: { type: "RangeExpression", fromIndex: { value:1}, toIndex: null } } ] );
    });
    it( 'should parse range specifier with empty start and end', function() { 
      comp( Parser.parse( "string[:]" ),
        [ { type: "ExpressionStatement", expression: { type: "RangeExpression", fromIndex: null, toIndex: null } } ] );
    });
    ...
    it( 'should parse object references (starting with number)', function() { 
      comp( Parser.parse( "return #0F623" ),
        [ { type: "ReturnStatement", argument: { type: "ObjRefLiteral", value: "#0F623" } } ] );
    });
    ...
    it( 'should parse single variable declarations', function() { 
      comp( Parser.parse( "Integer i;" ),
        [ { type: "VariableDeclaration", declarations: [ { type: "VariableDeclarator", name: { value: "i" }, dataType: { value: 'Integer' } } ] } ] );
    });
    it( 'should parse multi-variable declarations', function() { 
      comp( Parser.parse( "Integer i, j;" ),
        [ { type: "VariableDeclaration", declarations: [ { type: "VariableDeclarator", name: { value: "i" }, dataType: { value: 'Integer' } }, { type: "VariableDeclarator", name: { value: "j" }, dataType: { value: 'Integer' } } ] } ] );
    });
    it( 'should parse variable declarations with intializer', function() { 
      comp( Parser.parse( "Integer i = 23;" ),
        [ { type: "VariableDeclaration", declarations: [ { type: "VariableDeclarator", name: { value: "i" }, dataType: { value: 'Integer' }, init: { value: 23 } } ] } ] );
    });
    ...
    it( 'should parse variable declarations for all the builtin types', function() { 
        language_features.builtin_types.forEach( function(vType) { 
            comp( Parser.parse( vType + " i = x" ),
                [ { type: "VariableDeclaration", declarations: [ { type: "VariableDeclarator", name: { value: "i" }, dataType: { value: vType } } ] } ] );
        });
    } );
